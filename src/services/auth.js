...
export const requestResetToken = async (email) => {
  ...
  // зчитуємо з диска. ->  в рядок
  const templateSource = (
    await fs.readFile(resetPasswordTemplatePath)
  ).toString();

  // компіляція в шаблон-функцію
  ...
  // Надсилаємо ел лист користувачу
  try {
    await sendMail({
...
export const resetPassword = async (payload) => {
  ...
  const encryptedNewPassword = await bcrypt.hash(payload.token, 10);
  await UsersCollection.updateOne(
    { _id: user._id },
    { password: encryptedNewPassword },
  );

  await UsersCollection.deleteMany({ userId: user._id });
};
